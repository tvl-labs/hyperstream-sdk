...
const quotes = await client.quotes(quoteReq);

const bestRoute = quotes.routes[0];
if (!bestRoute) throw new Error("No routes available");

for await (const streamed of client.streamQuotes(quoteReq)) {
  console.log("stream route", streamed.routeId);
}
```

Key SDK helpers:

- `quotes` / `streamQuotes` — fetch or stream cross-chain routes.
- `buildDeposit` / `submitDeposit` — hydrate wallet steps and register deposits (`orderId`).
- `searchTokens`, `searchTokensPage`, `searchTokensWithAdapters`, `autocompleteTokens`.
- `getTokenBalances`, `getTopTokens`, `getMTokens`, `getChains`, `getArcadiaConfig`, `getVaults`.
- `getIntentStatus`, `getIntentByDeposit`, `getIntentsByAuthor`, `getOrders`.
...
for (const approval of deposit.approvals ?? []) {
  if (approval.request.method === "wallet_switchEthereumChain") {
    await ethereum.request(approval.request);
    continue;
  }

  const result = await ethereum.request(approval.request);
  if (typeof result === "string" && isHex(result)) {
    if (approval.waitForReceipt) {
      // Optional: wait for receipt using your client stack.
    }
    if (approval.deposit) depositTxHash = result as Hex;
  }
}
...
export interface GetTokenBalancesByAddressResponse = TokenWithBalance[];
```
